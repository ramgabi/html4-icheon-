function init(){
  ...
  if(srcBar.value=='검색어를 입력하세요'){
   ...
   return;
	...
	var galleryFrm=document.getElementsByName('gallery_frm')[0];
	var gallerySearch=galleryFrm.getElementsByTagName('input')[0];
	
	gallerySearch.onclick=function(){
		if(gallerySearch.value=="검색어를 입력해 주세요"){
			gallerySearch.value="";
		}
	}
