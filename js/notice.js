function init(){
  ...
  if(srcBar.value=='검색어를 입력하세요'){
   ...
   return;
	...
	var boardFrm=document.getElementsByName('board_frm')[0];
	var boardSearch=boardFrm.getElementsByTagName('input')[0];
	
	boardSearch.onclick=function(){
		if(boardSearch.value=="검색어를 입력해 주세요"){
			boardSearch.value="";
		}
	}
